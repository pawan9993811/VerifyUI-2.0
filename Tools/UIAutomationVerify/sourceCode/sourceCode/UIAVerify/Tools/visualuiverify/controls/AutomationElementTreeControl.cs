...
namespace VisualUIAVerify.Controls
{
    ...
    public partial class AutomationElementTreeControl : UserControl
    {
        ...
        static bool FindControl_Old(List<UIControl> childList, UIControl node, out int index)
        {
        ...
        {
            bool allPropsMatched = false;
            bool tempFlag = true; bool duplicateDisplayName = false;
            for (int i = 0; i < childList.Count; i++)
            {
                if (childList[i].Name.Contains(node.Name))
                {
                    tempFlag = true; duplicateDisplayName = true;
                    foreach (Property prop in node.SearchCriteria.SearchProperties)
                    {
                        tempFlag &= childList[i].SearchCriteria.SearchProperties.Any(p => p.Name.Equals(prop.Name));
                        tempFlag &= childList[i].SearchCriteria.SearchProperties.Any(p => p.Value.Equals(prop.Value));
                    }
                    if (tempFlag)
                    {
                        index = i;
                ...
                allPropsMatched = tempFlag;
            }
            index = -1;
            if (!allPropsMatched && duplicateDisplayName)
                node.Name = node.Name + new Random().Next(0, 999);
